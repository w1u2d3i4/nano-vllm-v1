...
class ModelRunner:
    ...
    def run_model(self, input_ids: torch.Tensor, positions: torch.Tensor):
        ...
        use_graph = (not self.enforce_eager and bs <= 512
                     and bs == num_seqs and context.block_tables is not None)
        if not use_graph:
            ...
            set_context(graph_vars["cu_seqlens_q"][:bs+1], graph_vars["cu_seqlens_k"][:bs+1],
                        context.max_seqlen_q, context.max_seqlen_k,
                        graph_vars["slot_mapping"], graph_vars["context_lens"],
                        graph_vars["block_tables"], context.seq_need_compute_logits)
            graph.replay()
    ...
    def capture_cudagraph(self):
        ...
        for bs in reversed(self.graph_bs):
            ...
            set_context(cu_seqlens_q[:bs+1], cu_seqlens_k[:bs+1], 1, 1, slot_mapping[:bs], context_lens[:bs], block_tables[:bs])
